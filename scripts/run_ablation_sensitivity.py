#!/usr/bin/env python3
"""
Run ablation and sensitivity experiments with 20 tasks × 2 runs.
Matches main experiment setup for consistency.
"""
...
def compute_coverage(views):
    """Count distinct level combinations visited."""
    seen = set()
    for v in views:
...
def run_navllm_session(task, schema, engine, llm_client, config, budget=12):
    """Run a single NavLLM session with given config."""
...
def run_ablation():
    """Run ablation study: 20 tasks × 2 runs for each configuration."""
    print("="*70)
    print("ABLATION STUDY (20 tasks × 2 runs)")
    print("="*70)
    
    output_file = RESULTS_DIR / "ablation_20tasks.csv"
    ...
    for task in tasks:
        ...
        for config_name, config in configs.items():
            for run in range(num_runs):
                ...
                try:
                    start = time.time()
                    result = run_navllm_session(task, schema, engine, llm_client, config)
...
def run_sensitivity():
    """Run sensitivity analysis: 20 tasks × 2 runs for each weight config."""
    print("\n" + "="*70)
    print("SENSITIVITY ANALYSIS (20 tasks × 2 runs)")
    print("="*70)
    
    output_file = RESULTS_DIR / "sensitivity_20tasks.csv"
    ...
    weight_configs = [
        ...
        (0.3, 0.5, 0.2),  # Medium pref
        (0.2, 0.6, 0.2),  # High pref
...
def summarize_results():
    ...
    # Ablation summary
    ablation_file = RESULTS_DIR / "ablation_20tasks.csv"
    if ablation_file.exists():
        ...
        print("\nAblation Summary (20 tasks × 2 runs = 40 sessions per config):")
        print(summary)
        summary.to_csv(RESULTS_DIR / "ablation_summary.csv")
    
    # Sensitivity summary
    sens_file = RESULTS_DIR / "sensitivity_20tasks.csv"
    if sens_file.exists():
        ...
        print("\nSensitivity Summary (20 tasks × 2 runs = 40 sessions per config):")
        print(summary)
...
if __name__ == "__main__":
    ...
    if args.summary:
        summarize_results()
    elif args.all or (not args.ablation and not args.sensitivity):
        run_ablation()
        run_sensitivity()
        summarize_results()
    else:

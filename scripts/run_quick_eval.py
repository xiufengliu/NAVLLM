#!/usr/bin/env python3
"""
Fast experiments using Ollama for LLM methods.
Runs all experiments: main baselines, ablation, sensitivity.
"""
...
# Use Ollama with qwen2.5:3b (good balance of speed and quality)
...
class NavLLMNav:
    def __init__(self, schema, engine, llm_client, config=None):
        config = config or RecommenderConfig(lambda_data=0.4, lambda_pref=0.4, lambda_div=0.2, top_k=3)
        self.rec = NavLLMRecommender(schema, engine, llm_client, config)
...
def run_main_experiments():
    """Run main baseline comparison: 20 tasks × 2 runs."""
    print("="*70)
    print("MAIN EXPERIMENTS (20 tasks × 2 runs)")
    print("="*70)
    
    output_file = RESULTS_DIR / "main_results_ollama.csv"
...
def run_ablation():
    """Run ablation study: 20 tasks × 2 runs."""
    print("\n" + "="*70)
    print("ABLATION STUDY (20 tasks × 2 runs)")
    print("="*70)
    
    output_file = RESULTS_DIR / "ablation_ollama.csv"
    ...
    for task in tasks:
        ...
        for config_name, config in configs.items():
            for run in range(num_runs):
                ...
                try:
                    nav = NavLLMNav(schema, engine, llm_client, config)
                    start = time.time()
                    result = run_session(nav, task, schema, engine)
                    elapsed = time.time() - start
...
def run_sensitivity():
    """Run sensitivity analysis: 20 tasks × 2 runs."""
    print("\n" + "="*70)
    print("SENSITIVITY ANALYSIS (20 tasks × 2 runs)")
    print("="*70)
    
    output_file = RESULTS_DIR / "sensitivity_ollama.csv"
...
def summarize_all():
    ...
    # Main results
    main_file = RESULTS_DIR / "main_results_ollama.csv"
    if main_file.exists():
        df = pd.read_csv(main_file)
        print("\nMain Results (20 tasks × 2 runs = 40 sessions per method):")
        ...
        print(summary)
        summary.to_csv(RESULTS_DIR / "main_summary.csv")
    
    # Ablation
    abl_file = RESULTS_DIR / "ablation_ollama.csv"
    if abl_file.exists():
        df = pd.read_csv(abl_file)
        print("\nAblation Results (20 tasks × 2 runs = 40 sessions per config):")
    ...
    # Sensitivity
    sens_file = RESULTS_DIR / "sensitivity_ollama.csv"
    if sens_file.exists():
        df = pd.read_csv(sens_file)
        print("\nSensitivity Results (20 tasks × 2 runs = 40 sessions per config):")
        summary = df.groupby('weights').agg({
            'hit_strict': 'mean',
            'cumulative_i_data': 'mean',
            'redundancy': 'mean'
        }).round(3)
        print(summary)
        summary.to_csv(RESULTS_DIR / "sensitivity_summary.csv")
...
if __name__ == "__main__":
    ...
    if args.summary:
        summarize_all()
    elif args.all or (not args.main and not args.ablation and not args.sensitivity):
